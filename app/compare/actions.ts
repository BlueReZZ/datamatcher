...
export async function getPublicationDetails(sourceDoi: string, matchDoi: string) {
  try {
    // Fetch both publications in parallel
    const [sourceResponse, matchResponse] = await Promise.all([
      fetch(`${CROSSREF_API_URL}/${encodeURIComponent(sourceDoi)}`),
...
function sanitizePublication(pub: Publication): Publication {
  return {
    ...
    // Keep the abstract as is - we'll format it when rendering
